...
const BookingModal = ({ treatment, date, setTreatment, refetch }) => {
    ...
    const handleBooking = event => {
        ...
        fetch('https://vast-bayou-22125.herokuapp.com/booking', {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                ...
                console.log(data);
                if (data.success) {
                    toast(`your appoint is set on ${formattedDate} at ${slot}`)

                }
                else {
                    toast.error(`your appointment have already placed on ${data.booking?.date} at ${data.booking?.slot}`)
                }
                // to close modal
                setTreatment(null)
                refetch();
